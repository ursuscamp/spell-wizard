...
test('shared types and server utilities define admin word review data', async () => {
  ...
  assert.match(sharedSource, /export interface AdminWordReviewEntry \{[\s\S]*exampleSentence: string[\s\S]*tags: string\[\][\s\S]*reviewStatus: WordReviewStatus[\s\S]*\}/)
  assert.doesNotMatch(sharedSource, /enunciationText: string/)
...
test('admin words page supports filtering, review actions, and tts testing', async () => {
  ...
  assert.match(pageSource, /async function playAdminSentence\(entry: AdminWordReviewEntry\)/)
  assert.match(pageSource, /@click="playAdminSentence\(entry\)"/)
  assert.match(pageSource, /await speakWord\(entry\.exampleSentence, \{[\s\S]*mode: 'standard'/)
  assert.match(pageSource, /const wordToSpeak = entry\.word/)
  assert.match(pageSource, /:disabled="!playbackEnabled \|\| activePlaybackKey === `\$\{entry\.id\}:enunciate`"/)
  assert.match(pageSource, /:disabled="!playbackEnabled \|\| activePlaybackKey === `\$\{entry\.id\}:sentence`"/)
  ...
  assert.match(pageSource, /availableVoices,/)
  assert.match(pageSource, /loadVoices,/)
  assert.doesNotMatch(pageSource, /voiceEnabled/)
  assert.doesNotMatch(pageSource, /selectedVoiceUri,/)
  assert.doesNotMatch(pageSource, /setSelectedVoiceUri,/)
  assert.match(pageSource, /const testerVoiceId = ref\(''\)/)
  assert.match(pageSource, /if \(import\.meta\.client && !availableVoices\.value\.length\) \{\s*await loadVoices\(\)/)
  assert.match(pageSource, /v-model="testerVoiceId"/)
  assert.match(pageSource, /voiceId: testerVoiceId\.value \|\| null/)
  assert.match(pageSource, /const playbackEnabled = computed\(\(\) => speechSupported\.value\)/)
  assert.doesNotMatch(pageSource, /:disabled="!speechReady && !availableVoices.length"/)
  assert.match(pageSource, />\s*Default voice\s*</)
  assert.match(pageSource, /@click="testTts"/)
  assert.match(pageSource, />\s*Pronounce\s*</)
  assert.match(pageSource, />\s*Slower\s*</)
  assert.match(pageSource, />\s*Read sentence\s*</)
  ...
  assert.doesNotMatch(pageSource, /<span class="tiny muted">Standard<\/span>/)
})
